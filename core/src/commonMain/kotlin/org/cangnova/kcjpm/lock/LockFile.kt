...
import org.cangnova.kcjpm.platform.Instant
import org.cangnova.kcjpm.platform.KPath
import org.cangnova.kcjpm.platform.getProcessExecutor
...
sealed class PackageSource {
    ...
    companion object {
        fun fromDependency(dep: Dependency, projectRoot: KPath): PackageSource {
            return when (dep) {
                is Dependency.RegistryDependency -> Registry(dep.registryUrl)
                is Dependency.PathDependency -> {
                    val relativePath = try {
                        projectRoot.path.let { root ->
                            val depPathStr = dep.path.path
                            if (depPathStr.startsWith(root)) {
                                depPathStr.substring(root.length).trimStart('/')
                            } else {
                                depPathStr
                            }
                        }
        ...
        private fun resolveGitCommit(localPath: KPath?): String {
            if (localPath == null) return "unknown"
            
            return try {
                val result = getProcessExecutor().execute(
